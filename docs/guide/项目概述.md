...
- **图床配置**：支持自定义图床（阿里云 OSS、腾讯云 COS、七牛云 Kodo），降低存储成本。
- **用户管理**：支持修改密码，增强账号安全性。
- **图片处理**：上传图片自动转换为格式并压缩，提升加载速度。
...
[![Deploy on Zeabur](https://zeabur.com/button.svg)](https://zeabur.com/templates/github?repo=xiongaox/ValPoint)

详细部署教程请参考 [部署指南](../dev/部署指南.md)
