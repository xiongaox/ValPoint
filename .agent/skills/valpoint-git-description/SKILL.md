...
description: 仅在 ValPoint 项目中使用；当用户输入"git描述"时，根据距离上次提交后的改动生成中文 Git Commit Message，并在生成后同步更新 src/changelog.ts。
---
...
7. 完成后做最小校验。
   - 重新读取 `src/changelog.ts` 的目标日期节，确认条目内容与 `logItems` 完全一致。
