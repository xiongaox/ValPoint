...
<figure class="full-bleed">
  <img class="full-bleed" src="../public/plate/SupaBase-01.jpg" alt="新建项目" />
  <figcaption>新建项目</figcaption>
</figure>

<figure class="full-bleed">
  <img class="full-bleed" src="../public/plate/SupaBase-02.jpg" alt="新建项目" />
...
| `map_pool_config` | 排位地图池配置（在池/回归/轮出）|
...
| `download_logs` | 下载日志 |
| `map_pool_config` | 地图池配置 |

## 🗺️ 快速更新地图池

> [!TIP]
> 每当游戏赛季更新（地图轮换）时，无需修改前端代码。只需在 SQL Editor 中执行以下脚本，即可一键更新全站地图角标状态。

::: details 点击查看地图池更新脚本（支持中文、增量更新）

```sql
-- 使用说明：在下方变量中填入地图中文名称
-- 增量更新：脚本仅修改你填写的地图，未提及的地图将保持原样（例如 Map A 上赛季是在池，本赛季不填它则依然是在池）
DO $$
DECLARE
    -- 【按需修改以下变量，支持多个地图用逗号隔开】
    v_returning    TEXT := '微风岛屿'; 
    v_rotated_out  TEXT := '日落之城,亚海悬城'; -- <--- 像这样写在单引号里
    -- v_in_pool      TEXT := '';         -- 【按需开启】设为在池状态（如需批量让回归地图转正）
    -- v_remove       TEXT := '';         -- 【按需开启】移除特定地图的所有状态角标
BEGIN
    -- 建立中英文临时映射表
    CREATE TEMP TABLE map_mapping (en TEXT, cn TEXT);
    INSERT INTO map_mapping VALUES 
      ('Ascent', '亚海悬城'), ('Abyss', '幽邃地窟'), ('Split', '霓虹町'), 
      ('Fracture', '裂变峡谷'), ('Bind', '源工重镇'), ('Breeze', '微风岛屿'), 
      ('Lotus', '莲华古城'), ('Sunset', '日落之城'), ('Pearl', '深海明珠'), 
      ('Icebox', '森寒冬港'), ('Corrode', '盐海矿镇'), ('Haven', '隐世修所');

    -- 1. 更新为 [回归]
    IF v_returning <> '' THEN
        INSERT INTO public.map_pool_config (map_name, pool_status)
        SELECT en, 'returning' FROM map_mapping WHERE cn = ANY(string_to_array(replace(v_returning, ' ', ''), ','))
        ON CONFLICT (map_name) DO UPDATE SET pool_status = 'returning', updated_at = NOW();
    END IF;

    -- 2. 更新为 [轮出]
    IF v_rotated_out <> '' THEN
        INSERT INTO public.map_pool_config (map_name, pool_status)
        SELECT en, 'rotated-out' FROM map_mapping WHERE cn = ANY(string_to_array(replace(v_rotated_out, ' ', ''), ','))
        ON CONFLICT (map_name) DO UPDATE SET pool_status = 'rotated-out', updated_at = NOW();
    END IF;

    -- 3. 更新为 [在池]
    IF v_in_pool <> '' THEN
        INSERT INTO public.map_pool_config (map_name, pool_status)
        SELECT en, 'in-pool' FROM map_mapping WHERE cn = ANY(string_to_array(replace(v_in_pool, ' ', ''), ','))
        ON CONFLICT (map_name) DO UPDATE SET pool_status = 'in-pool', updated_at = NOW();
    END IF;

    -- 4. 彻底移除角标状态
    IF v_remove <> '' THEN
        DELETE FROM public.map_pool_config 
        WHERE map_name IN (SELECT en FROM map_mapping WHERE cn = ANY(string_to_array(replace(v_remove, ' ', ''), ',')));
    END IF;

    DROP TABLE map_mapping;
    RAISE NOTICE '地图池增量同步成功！未涉及的地图状态已保留。';
END $$;
```
...
async function handleDouyin(targetUrl: string) {
  ...
  // 【新方法】从 meta description 提取用户名
  // 格式: [视频标题] - [用户名]于[日期]发布在抖音
