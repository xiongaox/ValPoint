 ...
 *
 * 主要导出：
 * - `default:AdminProfileModal`
 *
 * 依赖关系：
 * - 上游依赖：`react`、`../../../components/Icon`、`../../../components/UserAvatar`、`../../../supabaseClient`
...
const AdminProfileModal: React.FC<Props> = ({ isOpen, onClose, adminEmail, setAlertMessage, onProfileUpdate }) => {
    const [profile, setProfile] = useState<AdminProfile | null>(null);
    const [isProfileLoading, setIsProfileLoading] = useState(true);
    const [nickname, setNickname] = useState('');
    const defaultAvatar = adminEmail ? getPlayerCardByEmail(adminEmail) : getDefaultPlayerCardAvatar();
    ...
    useEffect(() => {
        ...
        const loadProfile = async () => {
            ...
            const { data, error } = await adminSupabase
                .from('user_profiles')
                .select('id, nickname, avatar, custom_id')
                .eq('id', user.id)
                .single();
            ...
            if (!error && data) {
                ...
                setCurrentAvatar(data.avatar || defaultAvatar);
                if (!data.custom_id) {
    ...
    const handleSave = async () => {
        ...
        if (!error) {
            updateAvatarCache(adminEmail, currentAvatar);
            setAlertMessage('个人信息已更新');
            onProfileUpdate?.(nickname || updateData.nickname, currentAvatar);
            onClose();
        } else {
            setAlertMessage(error.message || '更新失败');
        }
    };
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-gray-500">
                                关联邮箱: <span className="text-gray-300">{adminEmail}</span>
                            </span>
                        </div>
                        <input
                            type="text"
